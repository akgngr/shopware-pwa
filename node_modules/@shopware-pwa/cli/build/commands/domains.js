...
[CLI > domains] Synchronize the domain's related config from backend (in order to build a domains.json file)`);
    ...
    if ((!username || !password) && !toolbox.isDefaultDemoData()) {
      toolbox.print.error("[CLI > domains] Please provide your admin credentials using the --username and --password options or answering the questions.");
      return;
    }
    const authToken = await toolbox.auth.getAuthToken();
    if (!authToken) {
      toolbox.print.error(`[CLI > domains] Please try again. This synchronization is required.`);
      return -1;
    }
    let domainsMap = {};
    try {
      const domains = await toolbox.domains.fetchDomainsForSalesChannel(inputParameters.shopwareEndpoint, authToken, inputParameters.shopwareAccessToken);
    ...
    try {
      if (!Object.keys(domainsMap).length && toolbox.isDefaultDemoData()) {
        toolbox.print.warning("[CLI > domains] Loading default domains.json file.");
        const defaultDomainsMap = await toolbox.domains.getDefaultDemoDomainsJson();
