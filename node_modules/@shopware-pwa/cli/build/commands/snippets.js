...
module.exports = {
  ...
  run: async (toolbox) => {
    ...
    if (!toolbox.isProduction) {
      toolbox.print.warning(`[CLI > init] You're running CLI in development mode!`);
    }
    if (!isCIrun) {
    ...
    const languageConfig = path.join(".shopware-pwa", "sw-plugins", "languages.json");
    const languages = await toolbox.filesystem.readAsync(languageConfig, "json");
    if (!languages) {
      toolbox.print.error("[CLI > snippets] Language config does not exist. Pleas run 'yarn run shopware-pwa languages' first.");
      return;
    }
    const isoCodes = Object.keys(languages);
    const { username, password } = inputParameters;
    if (!username || !password) {
      toolbox.print.error("[CLI > snippets] Please provide your admin credentials using the --username and --password options.");
      return;
    }
    ...
    const snippetSetsMap = await toolbox.snippets.getSnippetSetsByLocales(isoCodes, inputParameters.shopwareEndpoint, authToken);
    ...
    if (!doExport) {
      toolbox.print.info("[CLI > snippets] Stopping after import (run with --export option to export Snippets to Shopware)");
      return;
    }
