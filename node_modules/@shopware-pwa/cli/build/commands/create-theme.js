...
var command = {
  ...
  run: async (toolbox) => {
    ...
    const execa = require_execa();
    if (!toolbox.isProduction) {
      toolbox.print.warning(`[CLI > create-theme] You're running CLI in development mode!`);
    }
    toolbox.print.info(`[CLI > create-theme] Creating new theme project...`);
    const themeNameAnswers = await toolbox.prompt.ask({
      type: "input",
      name: "themeName",
      message: "Type a name for your theme (needs to meet NPM package name validation rules):",
      initial: ""
    });
    const { themeName } = themeNameAnswers;
    const nameValidationResult = (0, import_validate_npm_package_name.default)(themeName);
    if (!nameValidationResult.validForNewPackages) {
      toolbox.print.error(`[CLI > create-theme] Cannot create new theme with name "${themeName}":`);
      (_a = nameValidationResult.warnings) == null ? void 0 : _a.forEach((warning) => toolbox.print.warning(`  -> ${warning}`));
      (_b = nameValidationResult.errors) == null ? void 0 : _b.forEach((error) => toolbox.print.error(`  -> ${error}`));
      process.exit(1);
    }
    ...
    const themeSelectOption = baseThemeAnswers.baseThemeSelect;
    ...
    await toolbox.patching.update(path.join(dirPath, "package.json"), (config) => {
    ...
    if (baseThemePackageName) {
      await execa("yarn", ["add", "-D", "@shopware-pwa/cli", baseThemePackageName], {
        stdio: "inherit",
        cwd: dirPath
      });
    }
    ...
    toolbox.print.success(`[CLI > create-theme] New theme generated! Go inside new folder and type "yarn run dev" to start creation.`);
  }
};
