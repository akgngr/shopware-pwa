...
var command = {
  ...
  run: async (toolbox) => {
    ...
    if (!toolbox.isProduction) {
      toolbox.print.warning(`[CLI > build-theme] You're running CLI in development mode!`);
    }
    const buildingSpinner = toolbox.print.spin("[CLI > build-theme] Building theme...");
    const destinationDirectoryName = "dist";
    const themePackage = await toolbox.filesystem.readAsync("package.json", "json");
    ...
    if (baseTheme) {
      buildingSpinner.text = "[CLI > build-theme] Running with base theme: " + baseTheme;
      try {
        const themePath = toolbox.getThemePath(baseTheme);
        await fse.copy(themePath, destinationDirectoryName, {
          dereference: true,
          filter: (src, dest) => {
            return !dest.includes("node_modules") && !dest.includes("package.json");
          }
        });
        const resultLocalesMap = {};
        const baseThemeLocalesMap = await toolbox.languages.getLocalesMap(path.join(themePath, "locales"));
        await toolbox.languages.mergeLocalesMap(resultLocalesMap, baseThemeLocalesMap);
        const localThemeLocalesMap = await toolbox.languages.getLocalesMap("locales");
        await toolbox.languages.mergeLocalesMap(resultLocalesMap, localThemeLocalesMap);
        await toolbox.languages.writeLanguages(resultLocalesMap, path.join(destinationDirectoryName, "locales"));
      } catch (e) {
