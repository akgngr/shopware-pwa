...
// src/extensions/plugins-extensions.ts
var import_axios = __toESM(require_axios2());
var import_path = require("path");
module.exports = (toolbox) => {
  toolbox.plugins = {};
  let runningRefreshPlugins = false;
  toolbox.plugins.invokeRefreshPlugins = async (devMode = false) => {
    var _a;
    if (runningRefreshPlugins) {
      return;
    }
    runningRefreshPlugins = true;
    await ((_a = toolbox == null ? void 0 : toolbox.runtime) == null ? void 0 : _a.run(`plugins`, { ci: true, devMode }));
    runningRefreshPlugins = false;
  };
  toolbox.plugins.getPluginsConfig = async (options = {}) => {
    if (options.localPlugins) {
      return toolbox.filesystem.readAsync(`sw-plugins/local-plugins.json`, "json");
    }
    return toolbox.filesystem.readAsync(`.shopware-pwa/pwa-bundles.json`, "json");
  };
  toolbox.fetchPluginsBuildArtifact = async ({
    shopwareEndpoint,
    authToken
  }) => {
    const pluginsConfigRsponse = await import_axios.default.post(`${toolbox.normalizeBaseUrl(shopwareEndpoint)}/api/_action/pwa/dump-bundles`, null, {
      headers: {
  ...
  toolbox.fetchPluginsConfig = async ({ config }) => {
    const endpoint = toolbox.normalizeBaseUrl(toolbox.inputParameters.shopwareEndpoint);
    const url = config.indexOf(endpoint) >= 0 ? config : toolbox.normalizeForwardSlashes(`${toolbox.normalizeBaseUrl(toolbox.inputParameters.shopwareEndpoint)}/${config}`);
    const pluginsConfigResponse = await import_axios.default.get(url);
    return pluginsConfigResponse.data;
  };
  toolbox.loadPluginsAssetFile = async ({ asset }) => {
    const endpoint = toolbox.normalizeBaseUrl(toolbox.inputParameters.shopwareEndpoint);
    const fileUrl = asset.indexOf(endpoint) >= 0 ? asset : toolbox.normalizeForwardSlashes(`${toolbox.normalizeBaseUrl(toolbox.inputParameters.shopwareEndpoint)}/${asset}`);
    ...
    const loadFile = function() {
      return new Promise((resolve, reject) => {
        request({
          ...
          if (err)
            reject(err);
          toolbox.filesystem.write(".shopware-pwa/pwa-bundles-assets.zip", body);
          resolve(true);
        });
      });
    };
  ...
  toolbox.unzipPluginsAssetsFile = async () => {
    ...
    const assetsFile = await unzipper.Open.file(".shopware-pwa/pwa-bundles-assets.zip");
    await assetsFile.extract({
      path: ".shopware-pwa/pwa-bundles-assets"
    });
  };
  toolbox.removeStringSpecialCharacters = (str) => str.replace(/[^\w\s]/gi, "-");
  toolbox.buildPluginsTrace = async ({
    ...
    const pluginsRootDirectory = rootDirectory || ".shopware-pwa/pwa-bundles-assets";
    const pluginsMap = Object.assign({}, pluginsTrace);
    if (pluginsConfig) {
      const pluginNames = Object.keys(pluginsConfig).filter((pluginName) => !(disabledPlugins == null ? void 0 : disabledPlugins.includes(pluginName)));
      pluginNames.forEach((pluginName) => {
        ...
        if (pluginDirExist) {
          const pluginConfig = toolbox.filesystem.read(`${pluginDirectory}/config.json`, "json");
          if (pluginConfig) {
            (_a = pluginConfig == null ? void 0 : pluginConfig.slots) == null ? void 0 : _a.forEach(async (slot) => {
              if (!pluginsMap[slot.name])
                pluginsMap[slot.name] = [];
              pluginsMap[slot.name].push(`~~/${pluginDirectory}/${slot.file}`);
            });
            if ((_b = pluginConfig == null ? void 0 : pluginConfig.layouts) == null ? void 0 : _b.length) {
              pluginConfig.layouts.forEach(async (layoutConfig) => {
                const slotName = toolbox.removeStringSpecialCharacters(`sw-layouts-${layoutConfig.name}`);
                if (!pluginsMap[slotName])
                  pluginsMap[slotName] = [];
                pluginsMap[slotName].push(`~~/${pluginDirectory}/${layoutConfig.file}`);
                const runtimeLayoutExist = toolbox.filesystem.exists(`.shopware-pwa/source/layouts/${layoutConfig.name}.vue`);
            ...
            if ((_c = pluginConfig == null ? void 0 : pluginConfig.pages) == null ? void 0 : _c.length) {
              pluginConfig.pages.forEach(async (pageConfig) => {
                const _a2 = pageConfig, { file, path } = _a2, params = __objRest(_a2, ["file", "path"]);
                const slotName = toolbox.removeStringSpecialCharacters(`sw-pages-${pageConfig.path}`);
                if (!pluginsMap[slotName])
                  pluginsMap[slotName] = [];
                pluginsMap[slotName].push(`~~/${pluginDirectory}/${pageConfig.file}`);
                const runtimePageExist = toolbox.filesystem.exists(`.shopware-pwa/source/pages/${pageConfig.path}.vue`);
                if (!runtimePageExist) {
                  await toolbox.template.generate({
                    template: `/plugins/PluginSlotTemplate.vue`,
  ...
  toolbox.buildPluginsMap = async (pluginsTrace) => {
    ...
    for (let index = 0; index < pluginSlotNames.length; index++) {
      ...
      if (slotComponents.length === 1) {
        ...
        const componentNames = slotComponents.map((component, index2) => {
          const arr = `${pluginSlotName}-${index2 + 1}`.split("-");
          let capital = arr.map((item, index3) => index3 ? item.charAt(0).toUpperCase() + item.slice(1).toLowerCase() : item);
          return capital.join("");
        });
        const bodyStart = "--> ";
        const bodyEnd = " <!--";
        const startingTags = [...componentNames].reverse().reduce((prev, current) => {
          return `${prev}<${current}>`;
        }, "");
        const endingTags = componentNames.reduce((prev, current) => {
          return `${prev}</${current}>`;
        }, "");
        const body = bodyStart + startingTags + endingTags + bodyEnd;
        const componentImports = slotComponents.reduce((prev, current, index2) => {
          return `${prev}
import ${componentNames[index2]} from '${current}'`;
        }, "");
        const components = slotComponents.reduce((prev, current, index2) => {
          return `${prev}
    ${componentNames[index2]},`;
        }, "");
        await toolbox.template.generate({
  ...
  toolbox.loadPluginsAssets = async () => {
    var _a, _b;
    const FETCH_ERROR_MESSAGE = "[CLI > plugins] Plugin settings are not fetched from shopware instance.";
    if (!toolbox.inputParameters.username || !toolbox.inputParameters.password) {
    ...
    try {
      ...
      const { buildArtifact, bundleConfig } = await toolbox.fetchPluginsBuildArtifact(__spreadProps(__spreadValues({}, toolbox.inputParameters), {
        authToken
      }));
      ...
      await toolbox.filesystem.removeAsync(`.shopware-pwa/pwa-bundles-assets.zip`);
      if (typeof bundleConfig === "undefined") {
        toolbox.print.divider();
        toolbox.print.info(toolbox.print.colors.bgYellow(toolbox.print.colors.black("Breaking change!")));
        toolbox.print.warning("The way to fetch the plugins' config has changed.\nPlease upgrade your SwagShopwarePwa plugin (>= v0.3.3) in Shopware 6 instance.");
        toolbox.print.warning("More info here: https://github.com/shopware/SwagShopwarePwa/commit/8cb521f54838fd4bc125ea2be4e10608f2b826de");
        toolbox.print.divider();
      }
      await toolbox.filesystem.writeAsync(".shopware-pwa/pwa-bundles.json", bundleConfig);
      await toolbox.loadPluginsAssetFile(buildArtifact);
      await toolbox.unzipPluginsAssetsFile();
    } catch (e) {
      if (((_a = e == null ? void 0 : e.response) == null ? void 0 : _a.status) === 401 || ((_b = e == null ? void 0 : e.response) == null ? void 0 : _b.status) === 403) {
        toolbox.print.error(`You provided bad cridentials for your shopware instance: ${toolbox.inputParameters.shopwareEndpoint} - plugins will not be added`);
      } else {
        toolbox.print.error(`UNEXPECTED ERROR ${(e == null ? void 0 : e.response) ? e.response : e}`);
        console.error(e);
      }
      return;
    }
  ...
  toolbox.createPluginsTemplate = async () => {
    ...
    const examplePluginDirectoryPath = (0, import_path.join)(localPluginsDirName, "my-local-plugin");
    await toolbox.filesystem.dirAsync(localPluginsDirName);
    await toolbox.filesystem.dirAsync(examplePluginDirectoryPath);
    const localPluginsConfigExists = await toolbox.filesystem.existsAsync((0, import_path.join)(localPluginsDirName, localPluginsConfigFilename));
    ...
    const localPluginConfigExists = await toolbox.filesystem.existsAsync((0, import_path.join)(examplePluginDirectoryPath, "config.json"));
    ...
    const localPluginVueFileExists = await toolbox.filesystem.existsAsync((0, import_path.join)(examplePluginDirectoryPath, "myLocalPlugin.vue"));
    ...
    const cmsReadmeExists = await toolbox.filesystem.existsAsync((0, import_path.join)(localPluginsDirName, "readme.md"));
    if (!cmsReadmeExists) {
      await toolbox.template.generate({
