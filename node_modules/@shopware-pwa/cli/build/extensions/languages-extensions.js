...
// src/extensions/languages-extensions.ts
module.exports = (toolbox) => {
  ...
  toolbox.languages.getLocalesMap = async (directoryPath) => {
    ...
    const languageKeys = await toolbox.languages.getDirLocalesList(directoryPath);
    const resultMap = {};
    languageKeys.forEach((languageKey) => {
      try {
        resultMap[languageKey] = toolbox.filesystem.read(path.join(directoryPath, languageKey), "json");
      } catch (e) {
    ...
    if (pluginsConfig) {
      ...
      pluginNames.forEach((pluginName) => {
        if (!pluginsConfig[pluginName])
          return;
        localesPaths.push(path.join(pluginsRootDirectory, pluginName, "locales"));
      });
    }
    return localesPaths;
  };
  let runningRefreshLanguages = false;
  toolbox.languages.invokeRefreshLanguages = async (isLocal = false) => {
    var _a;
    if (runningRefreshLanguages) {
      return;
    }
    runningRefreshLanguages = true;
    await ((_a = toolbox == null ? void 0 : toolbox.runtime) == null ? void 0 : _a.run(`languages`, { local: isLocal }));
    runningRefreshLanguages = false;
  };
  toolbox.languages.writeLanguages = async (languagesMap, destinationDirectoryName) => {
    const path = require("path");
    ...
    for (let index = 0; index < languageFileNames.length; index++) {
      ...
      const currentLocaleFilePath = path.join(destinationDirectoryName, currentLocaleFileName);
      await toolbox.filesystem.writeAsync(currentLocaleFilePath, languagesMap[currentLocaleFileName]);
    }
  };
};
/**
