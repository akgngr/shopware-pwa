...
// src/extensions/cms-extensions.ts
var import_path = require("path");
var import_lodash = __toESM(require_lodash());
module.exports = (toolbox) => {
  toolbox.cms = {};
  let runningRefreshCms = false;
  toolbox.cms.invokeRefreshCMS = async () => {
    ...
    await ((_a = toolbox == null ? void 0 : toolbox.runtime) == null ? void 0 : _a.run(`cms`, {}));
    runningRefreshCms = false;
  };
  toolbox.resolveCms = async (directoryPath, aliases, cmsComponentsMap) => {
    const readedMap = await toolbox.filesystem.readAsync((0, import_path.join)(directoryPath, "cmsMap.json"), "json");
    if (!readedMap)
      return;
    Object.keys(readedMap).forEach((sectionKey) => {
      Object.values(readedMap[sectionKey]).forEach((value) => {
        const alias = `sw-cms/${sectionKey}/${value}`;
        const path = (0, import_path.resolve)((0, import_path.join)(directoryPath, sectionKey, value));
        const configPathExists = toolbox.filesystem.exists(`${path}.vue`);
        if (configPathExists) {
          aliases[alias] = path.replace(/\\/g, "/");
        } else {
          toolbox.print.warning(`[cms] Invalid config for type ${alias} - destination file not exist in folder: ${directoryPath}`);
        }
      });
    });
    (0, import_lodash.merge)(cmsComponentsMap, readedMap);
  };
};
/**
 * @license
